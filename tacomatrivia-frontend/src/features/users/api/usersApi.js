...
export const usersApi = {
  ...
  create: async(body) => {
    const {headers,data} = await http.post('/users', body);
  ...
  edit: async(id,body) => {
    const {status} = await http.put(`/users/${id}`, body);
    
    console.log('Edited User, status:', status);
    return status;
  },
  delete: async(id) => {
    const {status} = await http.del(`/users/${id}`);
